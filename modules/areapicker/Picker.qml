...
MouseArea {
    ...
    function save(): void {
        ...
        CUtils.saveItem(screencopy, tmpfile, Qt.rect(Math.ceil(rsx), Math.ceil(rsy), Math.floor(sw), Math.floor(sh)), path => {
            const destFolder = Quickshell.env("HOME") + "/Pictures/Screenshots";
            const filename = "Screenshot_" + new Date().toISOString().replace(/[:.]/g, "-") + ".png";
            const destPath = destFolder + "/" + filename;
            Quickshell.execDetached(["sh", "-c", `mkdir -p '${destFolder.replace(/'/g, "'\\''")}' && cp '${path.replace(/'/g, "'\\''")}' '${destPath.replace(/'/g, "'\\''")}' && wl-copy --type image/png < '${destPath.replace(/'/g, "'\\''")}'`]);
            Quickshell.execDetached(["notify-send", "-a", "caelestia-cli", "-i", destPath, "Screenshot saved", "Saved to " + destPath + " and copied to clipboard"]);
            closeAnim.start();
        });
    }
